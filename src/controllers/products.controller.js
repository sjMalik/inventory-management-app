...
exports.getProducts = async (req, res) => {
    try {
        ...
        // Cache key
        const cacheKey = hashValue(`products:${status}:${search}:${page}:${limit}:${sort_by}:${order}`);
        // Check if the key exists in the cache
        const cachedData = await redisClient.get(cacheKey);

        if (cachedData) {
            return successResponse(res, JSON.parse(cachedData), 'Products retrieved successfully', 200);
        }

        const products = await Product.getAll({ status, search, page: parseInt(page, 10), limit: parseInt(limit, 10), sort_by, order });

        // Save the data in the cache 5 minutes
...
// Clear cache when produc changes
